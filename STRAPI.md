...
Strapi v5 is the primary headless CMS. It runs as a **separate Node.js sub-project** inside `cms/strapi/` with its own `node_modules`, its own `.env`, and its own PostgreSQL database (`scaffold_strapi`).
...
- **Strapi has its own database** (`scaffold_strapi`) on the same PostgreSQL server. It never touches the app database (`scaffold_dev`).
...
| `STRAPI_DB_NAME` | Database name | `"scaffold_strapi"` |
...
**Option B — Via code** (for the scaffold):

Create this directory structure:

```
cms/strapi/src/api/your-type/
├── content-types/your-type/schema.json   # Field definitions
├── routes/your-type.js                   # createCoreRouter('api::your-type.your-type')
├── controllers/your-type.js              # createCoreController('api::your-type.your-type')
└── services/your-type.js                 # createCoreService('api::your-type.your-type')
```

Use `cms/strapi/src/api/page/` as a reference. The route/controller/service files are boilerplate — they use Strapi's factory functions:
...
9. **Drop the database:** `dropdb scaffold_strapi`
